	...
	if($_POST){
		...
		if($file){
			...
			if($o->nombre != ''){
				...
				if($db->rows($sql_2) > 0){
					$HTML['data'] = 1;
					$HTML['tabla'] = ListaAgregarNiaAct($o->nia,$o->nombre,'No disponible','red');
				}else{
					$HTML['data'] = 1;
					$HTML['tabla'] = ListaAgregarNiaAct($o->nia,$o->nombre,'Disponible','green',$data2);
				}
				$db->liberar($sql_2);
			}else{
